...
# Файли
adaptive_log_path = "raft_benchmark_x.log"
basic_log_path = "./results/raft_benchmark_basic.log"
log_paths = {
    "node_one": "./results/node_50.log",
    "node_three": "./results/node_51.log"
}
JITTER_MS = 40
...
if not basic_data:
    print(f"No data found in {basic_log_path}")
    basic_times, basic_latencies, basic_errors = [], [], []
    basic_seconds, basic_base = [], None
else:
    min_len = min(len(basic_data), len(adaptive_data)) if adaptive_data else len(basic_data)
...
if not adaptive_data:
    print(f"No data found in {adaptive_log_path}")
    adaptive_times, adaptive_latencies, adaptive_errors = [], [], []
    adaptive_seconds, adaptive_base = [], None
else:
    min_len = min(len(basic_data), len(adaptive_data)) if basic_data else len(adaptive_data)
...
basic_delay_seconds, _ = to_relative_seconds(basic_delay_times) if basic_delay_times else ([], None)
adaptive_delay_seconds, _ = to_relative_seconds(adaptive_delay_times) if adaptive_delay_times else ([], None)

# --- Обчислення спільних меж для затримок ---
if basic_latencies and adaptive_latencies:
    combined_min_latency = min(min(basic_latencies or [0]), min(adaptive_latencies or [0]))
    combined_max_latency = max(max(basic_latencies or [0]), max(adaptive_latencies or [0]))
...
elif basic_latencies:
    combined_min_latency = min(basic_latencies)
    combined_max_latency = max(basic_latencies)
    y_margin = 0.05 * (combined_max_latency - combined_min_latency)
    shared_ylim = (combined_min_latency - y_margin, combined_max_latency + y_margin)
elif adaptive_latencies:
    combined_min_latency = min(adaptive_latencies)
    combined_max_latency = max(adaptive_latencies)
    y_margin = 0.05 * (combined_max_latency - combined_min_latency)
    shared_ylim = (combined_min_latency - y_margin, combined_max_latency + y_margin)
else:
    shared_ylim = (0, 100)  # Default if no data
    ...
    try:
        ...
        for match in pattern.findall(content):
            ...
            if adaptive_base:
                ...
                # Use current time as fallback if no adaptive_base
                relative_time = 0
        ...
        print(f"Error reading or processing {log_path}: {e}")
...
# Basic Raft
if basic_seconds and basic_latencies:
...
# Adaptive Raft
if adaptive_seconds and adaptive_latencies:
